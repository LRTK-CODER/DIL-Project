...
class Noise(DataSetting):
    ...
    def add(self, column:str, randomRange:list=[-9, 9]):
        ...
        result = [data + random.randrange(randomRange[0], randomRange[1]) for data in datas]
        
        self.datas[column] = result

    def multipleAdd(self, columns:list, randomRange:list=[-9, 9]):
        noiseList = [random.randrange(randomRange[0], randomRange[1]) for _ in range(0, len(self.datas))]
